...
static void run_timer_cmd(int argc, const char* const* argv) {
    struct timer_options opts = {
        .interval_ms = TIMER_DEFAULT_INTERVAL_MS,
        .do_stackdump = 0,
        .do_health = 0,
...
static void run_bt_backtrace_stackdump(int argc, const char* const* argv) {
...
static void run_command(char *line) {
    ...
    int argc = 0;
    char **_argv = NULL;
    if (args_parse(NULL, NULL, NULL, line, &argc, &_argv) != 0) {
    ...
    if (arg_is(argv[0], "do")) {
        if (argc < 2) {
            fprintf(stderr, OUT_HDR("=== actions (top overrides below) ===\n"));
            dbgaction_list(stderr, "");
            fprintf(stderr, OUT_HDR("=== end actions ===\n"));
        } else {
            if (dbgaction_do(argv[1], argv + 2, argc - 2) != 0)
                fprintf(stderr, "? action not found: %s\n", argv[1]);
        }
        goto out;
    }
    if (arg_is(argv[0], "exit")) {
        if (_argv) args_free(_argv, argc);
        if (s_exit_cb)
