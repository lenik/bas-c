...
#
# Environment:
#   BAS_C_SRCDIR       project root (default: packaging/../..)
#   BAS_C_FORCE_LOCAL  skip can-local check (set on a build host by gh-makerelease)
set -euo pipefail

_LIBDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
BAS_C_SRCDIR=${BAS_C_SRCDIR:-$(cd "$_LIBDIR/../.." && pwd)}
...
find_build_host_file() {
    ...
    for k in $(_kind_fallbacks "$kind"); do
        for base in "$BAS_C_SRCDIR/.config/bas-c" "$HOME/.config/bas-c"; do
...
cmd_dump() {
    local file=${1:-}
    if [[ -z $file ]]; then
        file=$(find_build_host_file "${BAS_C_PACKAGING:-win32}") || {
    ...
    if ((BH_N > 0)); then
        ...
        echo "last_shell=$(last_shell "${BAS_C_PACKAGING:-}")"
...
cmd_run() {
    ...
    local k
    for k in $(_kind_fallbacks "$kind"); do
        echo "    $BAS_C_SRCDIR/.config/bas-c/$k.build-host" >&2
        echo "    $HOME/.config/bas-c/$k.build-host" >&2
    done
    return 2
}
