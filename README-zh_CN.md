# bas-c
`bas-c` 是基于 Meson 的 **C 共享/静态库** 模板（`clib`），并附带示例命令行应用。  
构建 `libbas-c`（共享库与静态库）、安装头文件与 pkg-config，并提供示例应用 `bas-c`；同一仓库中可以继续添加更多应用。
...
`bas-c` 使用 `po/` 下的 gettext 翻译文件（`*.po` 与生成的 `.mo` 文件）。
...
- 开发态运行（`/build/bas-c`）若存在 `/build/po`，会优先使用项目内翻译资源。
...
```bash
LANGUAGE=ja /build/bas-c -h
LANGUAGE=zh_CN /build/bas-c -h
```
...
Copyright (C) 2026 Lenik <bas-c@bodz.net>
